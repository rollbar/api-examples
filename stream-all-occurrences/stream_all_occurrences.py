...
def stream_all_occurrences(project_read_token, min_id=None):
    ...
    if min_id is None:
        min_id = init_min_id(
            project_read_token
        )  # min id we have ever seen - stop streaming if we reach this id
...
def fetch_occurrences(project_read_token, last_id=None, min_id=0):
    ...
    resp = requests.get(url, headers={"X-Rollbar-Access-Token": project_read_token})
    result = resp.json()
    if result["err"] == 0:
